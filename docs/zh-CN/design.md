...
Suite 在线性流水线执行之上增加稳定的实验身份，不引入流水线之间的依赖。每个成员有自己的本地资源名映射，显式别名指向共享 Pool 实例。目录库保存定义摘要、提交成员关系和数据源耗尽状态，各数据子库拥有本地原子的任务及 visit 检查点。拆分存储不创建独立调度器，也不要求跨库检查点事务。历史 run 查询保留提交成员关系，但流水线载荷仍为当前状态，不是不可变的历史快照。
