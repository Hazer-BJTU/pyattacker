...
感知访问的契约以及[反向参考](reference.md#进阶反向遍历rewindretry-allvisits)；仅正向的遍历仍使用其既有的
...
载荷是可选的，且绝不驱动调度。
[反向参考](reference.md#进阶反向遍历rewindretry-allvisits) 规定了该接口，
而 [tutorial 第 16–17 步](tutorial.md#第-16-步--高级用回退和全部重试重新生成)
用一个可运行的程序把它搭了起来。
