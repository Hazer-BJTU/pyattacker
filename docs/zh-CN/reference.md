...
重访；打开未知级别的构建直接拒该存储；`visits-v1` 的 SQLite 存储武装写入者守卫，
...
那也是谱系不感知的写入者开始读不懂该存储的时刻，所以 `visits-v1` 的 SQLite 存储武装一个
**写入者守卫**：任何还没声明有访问谱系感知的连接对 `pipelines`、`tasks` 和 `artifacts` 做
`INSERT`/`UPDATE`/`DELETE` 都会响亮失败（`no such function:
