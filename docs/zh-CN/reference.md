...
`pyattacker.store.FactBatchStore` 是独立于必需 `Store` 协议的可选能力：
`write_facts(attempts, events)` 原子提交两个序列，成功后才回填 ID。
抛出异常时，批次不能有已提交行，传入记录的 ID 也必须保持不变。
该能力自行管理事务；若 SQLite 已有事务，调用会被拒绝，不会提交或回滚调用方的工作。
空批次不创建事务。

`WriteBehindStore` 优先使用这个能力。SQLite 实现了它，Suite 两种布局下包装的 SQLite
存储也一样。失败的原子批次保留在缓冲里，可以再次 flush。
分目录 Suite 中，较早成功的子库批次不会因后续子库失败而回滚；重试只写剩余批次。
连接淘汰先 flush、后移出缓存，因此 flush 失败时，子库连接和待写批次仍可用于重试。
检查点、工件和任务状态仍绕过缓冲同步写入；交接和访问事务的边界不变。

没有 `write_facts` 的旧存储仍通过逐条 `record_attempt`/`emit_event` 调用工作。
每次正常返回就从缓冲移除该条已确认记录。调用抛错时，原异常向上传播，失败的那条及尚未调用的后缀留在缓冲中。
但失败调用可能已经提交后才抛错，因此该包装器随后会拒绝继续写入事实，以及依赖 flush 的读取或刷新操作，
抛出以原异常为原因的 `StoreUnavailable`。`buffer_stats()["flush_blocked"]` 表示此状态；
`pending` 包含提交结果不确定的那条记录。重新构造包装器前，需要将已持久化记录与最初提交的事实核对，
不能直接重放待写后缀。对这类旧存储的模糊提交结果，框架不保证自动 exactly-once。
能保证原子回滚的自定义存储应实现可选批量能力。定制事实写入的 SQLite 子类还需要覆盖 `write_facts`，
或设置 `write_facts = None`，继续使用其逐条写入方法。

关闭时即使 flush 抛错也会释放底层存储，错误仍向上传播。
Suite 会尝试关闭所有子库、目录库和写入锁，并传播第一个错误。
关闭失败后应按需重新打开并核对记录，不能在已经关闭的连接上重试 flush。
...
| `export_rows(*, run_id=None)` | 嵌套的流水线行：含任务、工件和交接；每行都带 `attempts_total`，`merge_reports` 就是从它计数的 |
...
从行里读计数，会加强对自定义存储 `export_rows()` 的要求，所以这两个字段值得说清楚：

* **`attempts_total` 是必需的。** 它是流水线行的核心部分（`PipelineRecord.attempts_total`）；缺这个字段
  的行会抛 `ConfigError`，并指明是哪一行、来自哪个源，而不是悄悄按 `0` 计数。
* **嵌套的 `handoffs` 账本是可选的**，因为账本本身就是可选能力。行里没有嵌套它时，只要存储自己有
  `handoffs(pipeline_id=...)`，就从那里读——嵌套本来就来自同一个能力；两样都没有的存储就是没有跳转，
  记 `0`，这是实话。
...
| `events_total` | `source_events_total` 的废弃别名，只在对象上（1.0 移除） |
